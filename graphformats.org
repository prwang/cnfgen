...
** KTH predecessors list format — =simple=
   ...
   Ideally if a vertex $v$ has $u$  in its adjacency list then the $v$
   should be explicitly listed in the adjacency list of $v$. CNFgen is
   liberal and do not expect that  and edge $\{u,v\}$ considered to be
   in graph when either  $u$ is in the list of $v$, or  when $v$ is in
   the list of $u$, or both.
...
[[file:images/kthformatBI.png]]
   ...
   parser finds an adjacency list
