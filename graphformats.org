...
** KTH predecessors list format — bipartite graphs
   ...
   bipartition  is   greedily  computed   while  reading   the  graph.
   Let's first see two very explicative examples of the format
...
[[file:images/kthformatBI.png]]
   ...
   bipartite graph interpretation is as follows. When the parser finds
   a row

   : i : <j1> <j2> <j3> ... <jk> 0

   it checks whether any of the vertices has been already assigned
   to either the left  or the right side of the  graph, namely color 0
   and 1 respectively. If that is the case then the remaining vertices
   in the line can be colored in  a unique way. A conflict arises when
   a vertex $v$  has been already assigned by to  one color because of
   previous  lines, and  a  new line  forces $v$  to  the other color.
   In such case the parser gives error.

   In case no vertex in the line has a color, the index =i= gets color
   0  (i.e. left  side of  the  bipartite) and  =j1=, ...  , =jk=  get
   color 1.
   
   This choice is arbitrary and greedy,  hence it is possible that the
   parsing fails even if the graph is actually bipartite.
